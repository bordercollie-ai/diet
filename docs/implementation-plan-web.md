...
- **2026-08-19 — manual bundled-food action:** `.github/workflows/add-bundled-food.yml` accepts a JSON template with a required Chinese/default name, optional English and Japanese names, serving, and core nutrition values. `scripts/add_manual_food.py` validates the input, derives a stable ID, updates only `web/src/data/food.json`, and opens a review PR. **Checks:** `python3 scripts/test_add_manual_food.py` (4 passed). **Next ready phase:** W10; W7 remains in progress pending reviewed Chinese mappings.
