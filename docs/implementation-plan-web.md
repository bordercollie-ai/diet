...
- **2026-08-18 UI refinement:** the exercise control is a compact, icon-led summary row below the nutrition cards; its input appears only in an accessible edit dialog, with the full-width save button below it. The row omits a redundant unit suffix, and the icon blends into the row without its own background. **Files changed:** `web/src/{lib/i18n.svelte.ts,pages/SummaryPanel.svelte}` and `web/tests/App.test.ts`. **Checks:** `pnpm check` (0 errors / 0 warnings) and `pnpm run test:ui` (29 passed; existing Svelte `derived_inert` stderr warning). **Next ready phase:** W10.
